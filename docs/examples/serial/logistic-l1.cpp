...
int main(int argc, char *argv[]) {
  ...
  const string filename{"logistic-l1-momentum.csv"};
#elif defined NESTEROV
  algorithm::nesterov<double, int, prox::l1norm> alg;
  alg.boosting_parameters(0.9, 1 / L);
  const string filename{"logistic-l1-nesterov.csv"};
#elif defined ADAGRAD
  algorithm::adagrad<double, int, prox::l1norm> alg;
  const string filename{"logistic-l1-adagrad.csv"};
#elif defined ADAM
  ...
  const string filename{"logistic-l1-adam.csv"};
#endif
