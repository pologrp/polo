...
#include "polo/encoder/dynamic.hpp"
#include "gtest/gtest.h"

class EncoderDynamic : public polo::encoder::dynamic<double, int>,
                       public ::testing::Test {
protected:
  EncoderDynamic() : x{-5, 1, 12, -7, 0, 0, -100, 500, 6, -30} {}
  void SetUp() override {}
  void TearDown() override {}
  ~EncoderDynamic() override = default;

  const std::vector<double> x;
};

TEST_F(EncoderDynamic, FullGradient) {
  ...
  const std::vector<double> expected{0, 0, 0, 0, 0, 0, -l2norm, l2norm, 0, 0};

  for (size_t idx = 0; idx < x.size(); idx++)
    EXPECT_DOUBLE_EQ(actual[idx], expected[idx]);
}

TEST_F(EncoderDynamic, BlockCoordinate) {
  std::vector<double> actual(x.size());
  ...
  const std::vector<double> expected{0, 0, 0, 0, 0, 0, -l2norm, 0, 0, -l2norm};

  for (size_t idx = 0; idx < x.size(); idx++)
    EXPECT_DOUBLE_EQ(actual[idx], expected[idx]);
}

TEST_F(EncoderDynamic, Serialization) {
