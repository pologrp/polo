...
template <class float_t> struct affine : private none<float_t> {
  ...
  OutputIt project(const float_t step, InputIt1 xold_begin, InputIt1 xold_end,
    ...
    polo::utility::matrix::blas<float_t>::gemv(
        'N', m, d, 1, A.data(), m, temp2.data(), 1, -1, temp1.data(), 1);
    polo::utility::matrix::lapack<float_t>::pptrs('L', m, 1, H.data(),
                                                   temp1.data(), m);
    polo::utility::matrix::blas<float_t>::gemv(
...
private:
  void factorize() {
    ...
    for (std::size_t idx = 0; idx < d; idx++)
      polo::utility::matrix::blas<float_t>::spr('L', m, 1, &A[idx * m], 1,
                                                 H.data());

    polo::utility::matrix::lapack<float_t>::pptrf('L', m, H.data());
  }
  std::size_t m, d;
