...
template <class value_t, class index_t> struct custom {
  ...
  using func_t = value_t *(*)(const value_t *, const value_t *, value_t *,
                              void *);

  custom() = default;

  value_t *boost(const value_t *gprev_b, const value_t *gprev_e,
                 value_t *gcurr) {
    return compute_(gprev_b, gprev_e, gcurr, data_);
  }
